...
class PostModel extends AbstractModel {
    ...
    static update(currentUser, params, postId) {
    ...
    static async getAllPost(searchData) {
        ...
        if(searchData) {
            ...
            } else if (!searchData.title && searchData.createdUser) {
                creater = await UserModel.getByName(searchData.createdUser);
                for(var index in creater) {
                    result = await connection.query(`SELECT * FROM posts WHERE created_user_id ='${creater[index].id}' ORDER BY created_at desc`)
                }
            } else if(searchData.title && searchData.createdUser) {
                creater = await UserModel.getByName(searchData.createdUser);
                for(var index in creater) {
                    result = await connection.query(`SELECT * FROM posts WHERE created_user_id ='${creater[index].id}' AND title ='${searchData.title}' ORDER BY created_at desc`)
                }
            } else {
